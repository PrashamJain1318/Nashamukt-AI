    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
        <motion.div
          ...
          className="h-24 w-24 bg-primary/20 rounded-full flex items-center justify-center mb-8"
        >
    ...
    <div className="min-h-screen bg-background pt-24 pb-32 px-4 selection:bg-primary/20">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          >
            <Glass variant="card" className="p-6 h-full flex flex-col justify-between border-primary/20">
              <div>
        ...
        >
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
            {data.sevenDayPlan.map((day, i) => (
              <Glass key={i} variant="card" className="p-5 border-border/50 hover:border-primary/50 transition-colors">
        ...
        <motion.div
          ...
          className="flex justify-center mt-16 pt-8 border-t border-border/40"
        >
          <Button size="lg" onClick={() => navigate('/dashboard')} className="h-14 px-10 text-lg rounded-full shadow-xl shadow-primary/20">
            Go to My Dashboard <ChevronRight className="ml-2 h-5 w-5" />
          </Button>
        </motion.div>
...
function ScoreCard({ title, score, icon, color, delay }: { title: string, score: number, icon: React.ReactNode, color: string, delay: number }) {
  return (
    <motion.div
      ...
      transition={{ delay }}
    >
      <Glass variant="card" className="p-6 h-full flex flex-col justify-between">
        ...
        <div className="flex items-end justify-between">
          <div className="space-y-1">
            ...
            <p className={`text-4xl font-bold ${color}`}>{score}<span className="text-lg text-muted-foreground">/100</span></p>
          </div>
          ...
          <div className="relative h-16 w-16">
            <svg className="w-full h-full transform -rotate-90">
              ...
              <circle cx="32" cy="32" r="28" stroke="currentColor" strokeWidth="4" fill="transparent" strokeDasharray={`${(score / 100) * 175} 175`} className={color} />
            </svg>
          </div>
        </div>
      </Glass>
